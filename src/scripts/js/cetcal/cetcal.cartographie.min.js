...
$(document).ready(function() {
  ...
  for (var i = 0; i < icons_array.length; i++) {
    ...
    $('div#dropdown-filtres-cartographie-liste').append(
      '<a class="dropdown-item" href="#" style="font-family: \'Signika\' !important; color: #6C3012 !important;"' 
      + ' onclick="return false;" onmousedown="appliquerFiltreCartographie(\'' + icons_array[i].icon.options.type 
      + '\');">' 
      + '<img src="' + icons_array[i].icon.options.iconUrl + '" height="32" />&#160;&#160;'
      + icons_array[i].icon.options.type + '</a>'
    );
  }

});

function appliquerFiltreCartographie(filtre) {
  clusters.clearLayers();
  if (filtre === 'NULL') {
    for (var i = 0; i < marker_collection.length; i++) clusters.addLayer(marker_collection[i].mrkr);
  } else {
    for (var i = 0; i < marker_collection.length; i++) {
      if (marker_collection[i].type === filtre) clusters.addLayer(marker_collection[i].mrkr);
      else clusters.removeLayer(marker_collection[i].mrkr);
    }
  }
}