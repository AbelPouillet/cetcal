...
$('#modal-cet-recherche-avancee').on('show.bs.modal', function (event) {
  ...
  $('#rav-envoi-recherche-avancee').on('mousedown', function(e) {
    ...
    $('.rav-categorie-critere').each(function() {
      json_rav.categories.push([{ 'data': $(this).attr('data'), 'data-type': $(this).attr('data-type') }]);
    });
    json_rav.produits = [];
    ...
    $.ajax({
      ...
      success: function(json) { 
        ...
        for (var i in data) {
          ...
          if (data[i].adrCodePostal != 0) console.log(data[i].adrCodePostal);
          else console.log(data[i].adrfermeLtrl);
          console.log(data[i].nomferme);
          console.log('---------------------------------------------------------------------');*/
          var cp = data[i].adrCodePostal != 0 ? data[i].adrCodePostal : data[i].adrfermeLtrl;
          $('#resultats-recherche-avancee').append(
            '<p>' + cp + '|' + data[i].nomferme + '|' + data[i].nom + ' ' + data[i].prenom + '</p>'
          );
 ...
 */
var init_zoom_factor = 10;
var min_zoom_factor = 4;
var fly_to_zoom_factor = 16;
var icons_array = [];
