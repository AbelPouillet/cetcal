...
$(document).ready(function() {
  ...
  $('#rav-homepage-envoyer').on('mousedown', function() {
    ...
    $('.rav-hp-categories').each(function() {
      if ($(this).is(':checked')) json_rav.types.push($(this).val());
    });
    
    if (json_rav.commune.length > 0) fetchResultatsRechercheAvancee(json_rav);
  });
...
function fetchResultatsRechercheAvancee(json_rav) {
  $.ajax({
    ...
    success: function(json) { 
      ...
      for (var i in data) {
        $('#resultats-recherche-avancee').append(
          '<div id="bloc-rav-' + data[i].pk + '" class="bloc-rav-element-resultat">'
            ...
            + ((data[i].prodInscrit === 'false') ? '[TODO thomas pour 11 oct]' : data[i].typeDeProduction.replaceAll('µ', ''))
      ...
      $('#zone-homepage-recherche-avancee').hide();
...
function lancerRechercheAvancee() {
  ...
  //$('#cet-annuaire-recherche-communes-conatiner .typeahead').attr('placeholder', 'Rechercher votre commune');
