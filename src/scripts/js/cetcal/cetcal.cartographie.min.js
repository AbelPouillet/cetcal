...
var icons_array = [];
$('#cet-annuaire-crt-main').height(460);
var htmlMiniFiche = '';
var cetcrtmainmap = L.map('cet-annuaire-crt-main').setView([44.854, -0.043], 13.1);
var clusters = L.markerClusterGroup();
var cetAnnuaireIcon = L.icon({
    iconUrl: 'res/content/icons/bonpour1tour/_ferme.png',
    iconSize: [38, 38],
    popupAnchor: [0, -20]
});
icons_array.push({icon: cetAnnuaireIcon, desc: 'Producteur référencé dans l\'annuaire cetcal.'});
var cetAnnuaireIcon_fournisseurcet = L.icon({
    iconUrl: 'res/content/icons/ferme-rouge_fcet.png',
    iconSize: [50, 74],
    popupAnchor: [0, -20]
});
...
var cetAnnuaireMarcheIcon = L.icon({
    iconUrl: 'res/content/icons/bonpour1tour/_magasin2.png',
    iconSize: [38, 38],
    popupAnchor: [0, -20]
});
icons_array.push({icon: cetAnnuaireMarcheIcon, desc: 'Marché référencé dans l\'annuaire cetcal.'});
var amapIcon = L.icon({
    iconUrl: 'res/content/icons/farmers-hand03.png',
    iconSize: [38, 38],
    popupAnchor: [0, -20]
});
...
$('producteur').each(function(i) {
    ...
    var htmlMiniFiche = '<p class="cet-cartographie-popup">' + 
        '<a class="cet-ann-crt-head-text" href="#">' + nomFerme + '</a><br>' + adr;
    ...
    marker.bindPopup(popup).on('popupopen', function() {
        ...
        $('#cet-annuaire-crt-controls-container').removeClass('col-lg-9').addClass('col-lg-12');
    }).on('popupclose', function() {
        $('#cet-annuaire-crt-mini-fiche-producteur').html('');
        $('#cet-annuaire-crt-mini-fiche-producteur-container').hide();
        $('#cet-annuaire-crt-controls-container').removeClass('col-lg-12').addClass('col-lg-9');
    });

    // Append to cluster marker group.
...
$('entite').each(function(i) {
    ...
    var marker = L.marker([coordinatesLl[1], coordinatesLl[0]], {icon: entite_icon});
    var popupOptions = {};
    var popup = L.popup(popupOptions, marker);
...
$(document).ready(function() {
  ...
  $('#valider-recherche-commune-cartographie').on('mousedown', function() {
    ...
    if (found === false) {
        ...
        $('#modal-questionaire-paragraphe').text('Nous ne trouvons aucune commune pour "' + c + 
            '". Veuillez sélectionner une commune à proximité de la votre.');
        $('#modal-questionaire-paragraphe-bis').text('Vous pouvez également nous signaler ce manquement ' + 
            ' dans nos données - pour cela, veuillez nous contacter par email ou sur les réseaux sociaux.');
        $('#modal-questionaire-paragraphe-ter').text('- L\'équipe CAL !');
  ...
  for (var i = 0; i < icons_array.length; i++) {
    $('#liste-icones-fonctions').append('<li><img style="width:38px;" src="' 
