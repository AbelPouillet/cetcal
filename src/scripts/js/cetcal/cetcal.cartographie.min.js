  ...
  $('#rav-envoi-recherche-avancee').on('mousedown', function(e) {
    ...
    $.ajax({
      ...
      success: function(json) { 
        ...
        var pk_rav_entites = [];       
        for (var i in data) {
          var cp = data[i].adrCodePostal != 0 ? data[i].adrCodePostal : data[i].adrfermeLtrl;
          $('#resultats-recherche-avancee').append(
            '<p id="rav-' + data[i].pk + '" class="rav-element-producteur">' + cp + '|' + data[i].nomferme + '|' + data[i].nom + ' ' + data[i].prenom + '|' 
        ...
        updateCartoRechercheAvancee(pk_rav_producteurs, pk_rav_entites);
      },
      error: function(jqXHR, textStatus, errorThrown) {
        $('#resultats-recherche-avancee').empty();
...
function updateCartoRechercheAvancee(pk_prds, pk_ents) {
  console.log('___updateCartoRechercheAvancee. Markers=' + marker_collection.length);
  ...
  for (var i = 0; i < marker_collection.length; i++) {
    if (marker_collection[i].type.indexOf('Producteur') !== -1) {
      ...
      if (pk_prds.includes(pk) && !loaded_pks.includes(pk)) {
        loaded_pks.push(pk);
