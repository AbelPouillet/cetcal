...
$('#modal-cet-recherche-avancee').on('show.bs.modal', function (event) {
  ...
  $('#rav-categorie-ajouter').on('mousedown', function() {
    if ($("#rav-categorie").find(":selected").val() != 0) {
      var this_id = 'rav-categorie-' + $("#rav-categorie").find(":selected").val().split(';')[0];
  ...
  $('#rav-envoi-recherche-avancee').on('mousedown', function(e) {
    if ($('#cet-annuaire-recherche-communes-value').val().length < 1) {
      e.preventDefault();
      var text = 'Le formulaire de contact est incomplet.';
      text += ' Il vous faut au moins renseigner une commune afin de pouvoir lancer la recherche avancée.';
