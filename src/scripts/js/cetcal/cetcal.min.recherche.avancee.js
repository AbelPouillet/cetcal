...
function fetchResultatsRechercheAvancee(json_rav) {
  $.ajax({
    ...
    success: function(json) { 
      ...
      $('#resultats-recherche-avancee').append((data.length <= 0) ? '<p class="detail-rav-resultats">Aucun résultat.</p>' : '<p class="detail-rav-resultats">' + data.length + ' résultats trouvés.</p>'); 
      for (var i in data) {
        $('#resultats-recherche-avancee').append(
          '<div id="bloc-rav-' + data[i].pk + '" class="bloc-rav-element-resultat">'
            + '<div class="row card-last-prd mt-3" onmousedown="ravElementFocus(' + data[i].pk + ');">'
      ...
      if (data.length > 0) focusOnBounds(pk_rav_producteurs);
    },
